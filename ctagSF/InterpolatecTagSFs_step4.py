...
central_SF_file = ROOT.TFile(basedir+"/"+centraldir+"/DeepCSV_cTag_SFs_94X.root","READ")
histos_names = [i.GetName() for i in central_SF_file.GetListOfKeys()]
print histos_names

smoothed_SF_file = ROOT.TFile(basedir+"/"+centraldir+"/DeepCSV_cTag_SFs_smooth_94X.root","RECREATE")
...
for hist_name in histos_names:
    ...
    xx = np.arange(0,1.+(max_-min_)/float(bins_),(max_-min_)/float(bins_))
    #print "-->",xx
    xxx = np.asarray([i+((max_-min_)/float(2*bins_)) for i in xx])
    #print "-->",xx
    #yy = np.arange(0,1,0.05)
    yy = np.arange(0,1.+(max_-min_)/float(bins_),(max_-min_)/float(bins_))
    ...
    if "central" in hist_name:
        ...
        outhist.GetZaxis().SetRangeUser(0.,1.8)
