...
def main():
    ...
    if not args.infiles == "*": 
        ...
        filelist = [f for f in os.listdir(indir) if not "TTTo2L2Nu" in f and not "ttHJet" in f]
    ...
    for f in filelist:
        ...
        if args.nmaxevtsperjob > 0 and nevts_dict[f] > args.nmaxevtsperjob:
            ...
            for i in range(len(eventsList)-1):
                ...
                flaunch_.write("python Analyze.py --infile=%s --outfile=%s --topmatchingdir=%s --tthfselectordir=%s --reweightingdir=%s --firstEvt=%i --lastEvt=%i --splitted=1 \n"%(indir+f,workingdir+"/SELECTED_"+args.tag+"/"+f,args.topmatchingdir,args.tthfselectordir, args.reweightingdir, eventsList[i], eventsList[i+1]))
                flaunch_.close()
            ...
            flaunch_.write("root -l setup.C \n")
            flaunch_.write("cd /user/smoortga/Analysis/2017/ttcc_Analysis/CMSSW_8_0_25/src/ttcc/analyse \n")
            flaunch_.write("python Analyze.py --infile=%s --outfile=%s --topmatchingdir=%s --tthfselectordir=%s --reweightingdir=%s --firstEvt=0 --lastEvt=%i --splitted=0 \n"%(indir+f,workingdir+"/SELECTED_"+args.tag+"/"+f,args.topmatchingdir,args.tthfselectordir, args.reweightingdir, nevts_dict[f]))
            flaunch_.close()  
