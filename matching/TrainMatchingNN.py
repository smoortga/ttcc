...
#     sys.exit(1)
    ...
    if args.TrainingFile == "":
        ...
        if args.nepoch>0:
            ...
            train_history_phase1 = model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=first_phase_nepoch, validation_data=(X_test, Y_test), callbacks = [ModelCheckpoint(os.getcwd() + "/"+args.tag+"/model_checkpoint_save.hdf5")], shuffle=True,verbose=args.verbose)
            pickle.dump(train_history_phase1.history,open(os.getcwd() + "/"+args.tag+"/loss_and_acc_phase1.pkl",'wb'))
            train_history_phase2 = model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=second_phase_nepoch, validation_data=(X_test, Y_test), callbacks = [ModelCheckpoint(os.getcwd() + "/"+args.tag+"/model_checkpoint_save.hdf5")], shuffle=True,verbose=args.verbose, sample_weight = w_train)
